...
class LRScheduler:
    def __init__(self, name, lr, iters_per_epoch, total_epochs, **kwargs):
        ...
        self.lr = lr    # 0.01 / 64.0 * 24
        self.iters_per_epoch = iters_per_epoch  # len(train_loader) 训练集图片数
        self.total_epochs = total_epochs        # opt.num_epochs = 30
        self.total_iters = iters_per_epoch * total_epochs

        self.__dict__.update(kwargs)

        self.lr_func = self._get_lr_func(name)  # name = "yoloxwarmcos"
